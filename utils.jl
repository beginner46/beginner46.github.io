...
@env function biography(md; resume="")
    io = IOBuffer()
    write(io, html("""<h1>About Me</h1>""") * md)
    isempty(resume) || write(io, html("""
        <p><i class="fas fa-download pr-1 fa-fw"></i>Download my <a href="$resume" target=_blank>resumé</a>.</p>"""))
    return String(take!(io))
end
